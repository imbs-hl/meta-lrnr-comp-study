...
res_indep_combalanced_megepro_mean_pewa_wa_na_keep <- res_indep_combalanced_megepro_wa_na_keep[ , .(mean_pewa = mean(meta_layer)), 
...
## na_action = na_impute
## -----------------------------------------------------------------------------
reg_megepro_indep_combalanced_na_impute <- wrap_batchtools(reg_name = "02-train-wa-na-imp",
                                                        work_dir = working_dir,
                                                        reg_dir = reg_indep_combalanced_megepro,
                                                        r_function = single_run_wa,
                                                        vec_args = data.frame(
...
## ----------------------------------------------
##
reg_indep_combalanced_megepro_wa_na_impute <- batchtools::loadRegistry(
  file.dir = file.path(reg_indep_combalanced_megepro, "02-train-wa-na-impute"),
  writeable = TRUE,
  conf.file = config_file)
reg_indep_combalanced_megepro_wa_na_impute <- batchtools::reduceResultsList(
  ids = batchtools::findDone(
    ids = 1:nrow(indep_combalanced_megepro_param_data),
    reg = reg_indep_combalanced_megepro_wa_na_impute
  ),
  reg = reg_indep_combalanced_megepro_wa_na_impute)


## resume filtered results
res_indep_combalanced_megepro_wa_na_impute <- data.table::rbindlist(reg_indep_combalanced_megepro_wa_na_impute)
res_indep_combalanced_megepro_mean_perf_wa_na_impute <- res_indep_combalanced_megepro_wa_na_impute[ , .(mean_perf = mean(meta_layer)), 
                                                                                              by = .(perf_measure, effect)]
print(res_indep_combalanced_megepro_mean_perf_wa_na_impute)
res_indep_combalanced_megepro_mean_perf_wa_na_impute$Setting <- "Independent"
res_indep_combalanced_megepro_mean_perf_wa_na_impute$Y_Distribution <- "Balanced"
res_indep_combalanced_megepro_mean_perf_wa_na_impute$Na_action <- "na.keep"
res_indep_combalanced_megepro_mean_perf_wa_na_impute$DE <- "DE: MeGe"
res_indep_combalanced_megepro_mean_perf_wa_na_impute$Meta_learner <- "BM"
saveRDS(
  object = res_indep_combalanced_megepro_mean_perf_wa_na_impute,
  file = file.path(res_indep_megepro,
                   "res_indep_combalanced_megepro_mean_perf_wa_na_impute.rds")
)
