...
## na_action = na_keep
## -----------------------------------------------------------------------------
reg_mege_indep_missbalanced_na_keep <- wrap_batchtools(reg_name = "02-train-wa-na-keep",
...
## ----------------------------------------------
##
reg_indep_missbalanced_mege_wa_na_keep <- batchtools::loadRegistry(
  file.dir = file.path(reg_indep_missbalanced_mege, "02-train-wa-na-keep"),
  writeable = TRUE,
  conf.file = config_file)
reg_indep_missbalanced_mege_wa_na_keep <- batchtools::reduceResultsList(
  ids = batchtools::findDone(
    ids = 1:nrow(indep_missbalanced_mege_param_data),
    reg = reg_indep_missbalanced_mege_wa_na_keep
  ),
  reg = reg_indep_missbalanced_mege_wa_na_keep)
...
## na_action = na_impute
## -----------------------------------------------------------------------------
reg_mege_indep_missbalanced_na_impute <- wrap_batchtools(reg_name = "02-train-wa-na-imp",
                                                       work_dir = working_dir,
                                                       reg_dir = reg_indep_missbalanced_mege,
                                                       r_function = single_run_wa,
                                                       vec_args = data.frame(
...
reg_indep_missbalanced_mege_wa_na_impute <- batchtools::reduceResultsList(
  ids = batchtools::findDone(
    ids = 1:nrow(indep_missbalanced_mege_param_data),
    reg = reg_indep_missbalanced_mege_wa_na_impute
  ),
  reg = reg_indep_missbalanced_mege_wa_na_impute)


## resume filtered results
res_indep_missbalanced_mege_wa_na_impute <- data.table::rbindlist(reg_indep_missbalanced_mege_wa_na_impute)
res_indep_missbalanced_mege_mean_perf_wa_na_impute <- res_indep_missbalanced_mege_wa_na_impute[ , .(mean_perf = mean(meta_layer)), 
                                                                                            by = .(perf_measure, effect)]
print(res_indep_missbalanced_mege_mean_perf_wa_na_impute)
