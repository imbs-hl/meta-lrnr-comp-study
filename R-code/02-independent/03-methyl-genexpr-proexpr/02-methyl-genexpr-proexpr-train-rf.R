...
## ----------------------------------------------
##
reg_megepro_train_rf <- batchtools::loadRegistry(
  file.dir = file.path(reg_indep_methyl_genexpr_proexpr, "02-train-rf"), writeable = TRUE,
  conf.file = config_file)
reg_megepro_train_rf <- batchtools::reduceResultsList(
  ids = batchtools::findDone(
    ids = 1:nrow(indep_methyl_genexpr_proexpr_param_data),
    reg = reg_megepro_train_rf
  ),
  reg = reg_megepro_train_rf)


## resume filtered results
reg_megepro_train_rf_DT <- data.table::rbindlist(reg_megepro_train_rf)
megepro_mean_perf_rf <- reg_megepro_train_rf_DT[ , .(mean_perf = mean(meta_layer)), 
