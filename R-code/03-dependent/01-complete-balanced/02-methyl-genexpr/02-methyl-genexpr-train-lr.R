...
## Send jobs
no.threads <- 5
reg_dep_mege_train_lr <- wrap_batchtools(reg_name = "02-train-lr",
...
## ----------------------------------------------
##
reg_dep_mege_train_lr <- batchtools::loadRegistry(
  file.dir = file.path(reg_dep_methyl_genexpr, "02-train-lr"),
  writeable = TRUE,
  conf.file = config_file)
reg_dep_mege_train_lr <- batchtools::reduceResultsList(
  ids = batchtools::findDone(
    ids = 1:nrow(dep_methyl_genexpr_param_data),
    reg = reg_dep_mege_train_lr
  ),
  reg = reg_dep_mege_train_lr)


## resume filtered results
dep_res_mege_lr <- data.table::rbindlist(reg_dep_mege_train_lr)
dep_mege_mean_perf_lr <- dep_res_mege_lr[ , .(mean_perf = mean(meta_layer)), 
                                                by = .(perf_measure, effect)]
print(dep_mege_mean_perf_lr)
dep_res_mege_lr$Setting <- "Dependent"
dep_res_mege_lr$DE <- "DE: MeGe"
dep_res_mege_lr$Meta_learner <- "LogReg"
saveRDS(object = dep_res_mege_lr,
